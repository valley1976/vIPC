...
#include <valley/base/lang/cleanup.h>
...
public:
    ...
    auto apply(Fn&& fn) -> decltype(std::forward<Fn>(fn)(std::declval<T&>()))
    {
        std::lock_guard<Mutex> guard{ mutex_ };

        return std::forward<Fn>(fn)(object_);
    }

    template<typename Fn>
    auto apply(Fn&& fn) const -> decltype(std::forward<Fn>(fn)(std::declval<const T&>())) {
        std::lock_guard<Mutex> guard{ mutex_ };
    ...
    bool try_apply(Fn&& fn)
    {
        if (mutex_.try_lock())
        {
            auto unlock = base::make_cleanup([this]() { mutex_.unlock(); });
    ...
    bool try_apply(Fn&& fn, Result& result)
    {
        if (mutex_.try_lock())
        {
            auto unlock = base::make_cleanup([this]() { mutex_.unlock(); });

            result = std::forward<Fn>(fn)(object_);
    ...
    template<typename Fn, typename Result>
    bool try_apply(Fn&& fn, Result& result) const
    {
        if (mutex_.try_lock())
        {
            auto unlock = base::make_cleanup([this]() { mutex_.unlock(); });
