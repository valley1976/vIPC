...
{
    assert(sizeof(T) <= max_data_size());
    return reinterpret_cast<const T*>(read_latest());
}
...
inline bool Channel::Subscriber::read_latest(T& value)
{
    ...
    auto* ptr = reinterpret_cast<T*>(read_latest());
    if (ptr) {
        value = *ptr;
        return true;
    }
...
{
    assert(sizeof(T) <= max_data_size());
    return reinterpret_cast<const T*>(read_earliest());
}
...
inline bool Channel::Subscriber::read_earliest(T& value)
{
    ...
    auto* ptr = reinterpret_cast<T*>(read_earliest());
    if (ptr) {
        value = *ptr;
        return true;
    }
