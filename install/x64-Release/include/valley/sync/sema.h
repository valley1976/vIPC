...
public:
    ...
    Semaphore(const char* name, int initialCount = 0)
    {
        assert(initialCount >= 0);

        char alias[256];
        snprintf(alias, 256, "vIPC_%s", name);

        m_hSema = CreateSemaphoreA(NULL, initialCount, MAXLONG, alias);
    }
    ...
    {
        return WaitForSingleObject(m_hSema, milliseconds) == WAIT_OBJECT_0;
    }
...
public:
    ...
    Semaphore(const char* name, int initialCount = 0)
    {
        ...
        m_sema = sem_open(alias, O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH, initialCount);
    }

    ~Semaphore()
    {
        if (m_sema == &m_inter_sema)
            sem_destroy(m_sema);
        else
            sem_close(m_sema); // named
    }
    ...
    bool timed_wait(int milliseconds)
    {
        ...
        int r = clock_gettime(CLOCK_REALTIME, &timeout);
        if (r == -1) return r;
        ...
        return sem_timedwait(m_sema, &timeout) == 0;
    }
