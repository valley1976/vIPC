...
class LIBVALLEY_BASE_EXPORT Fixed_streambuf : public std::streambuf
{
public:
...
class LIBVALLEY_BASE_EXPORT Logger final
{
public:
    Logger(Log_severity level, int line, const char* file, const char* func);
    ~Logger();
...
class LIBVALLEY_BASE_EXPORT Null_logger
{
public:
    Null_logger(Log_severity level, int line, const char* file)
    {};
...
template<int Log_severity, bool Enable = (Log_severity >= VALLEY_ACTIVE_LOG_LEVEL)>
struct Use_logger
{
    using type = Logger;
};

template<int Log_severity>
struct Use_logger<Log_severity, false>
{
